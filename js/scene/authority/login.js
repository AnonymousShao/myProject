...
var loginFn={
    ...
    init:function(fn){
        ...
        t.checkLogin();//登录
        t.weixin();//微信登录
        t.checkBidLogin();//是否显示什么是唯医通行证
        t.close();//页面关闭
    },
    //是否显示什么是唯医通行证
    checkLoginNum:function () {
        var num =localStorage.getItem("loginNum")?localStorage.getItem("loginNum"):0;
        var resultNum = parseInt(num);
        var permitObj = $(".ev-permit");
        if (resultNum>2){
            permitObj.hide();
        }else{
            permitObj.show();
    ...
    finPas:function(){
        ...
        $(".findPassword").off("click").on("click",function(){
            commLog.creatEvent({"id":4,"url":window.location.href,"keyword":"找回密码"});
            $(".ev-login").css("display","none");
            $(".ev-findPassword").css("display","block");
            commLog.createBrowse(5, "登录注册-找回密码页");
            $(".authorityBg").css("display","block");
        });
    ...
    fidCheck:function(){
        var t = this;
        $(".ev-fidCode,.ev-findNum").on("blur",function(){
            t.fidCheckCode2();
            t.fidActive();
        });
        $(".ev-fidCode,.ev-findNum").on("keyup",function(){
            t.fidCheckCode();
            t.fidActive();
        });
    },
    ...
    fidNextBtn:function(){
        ...
        $(".ev-fidNext").off("click").on("click",function(){
            ...
            if($(this).is(".activation")){
                commLog.creatEvent({"id":152,"url":window.location.href,"keyword":"找回密码下一步"});
    ...
    increment:function(){
        if(localStorage.getItem("loginNum")){
            var loginNum = parseInt(localStorage.getItem("loginNum"));
            loginNum++;
    ...
    savePass:function(){
        ...
        $(".ev-passSave").off("click").on("click",function(){
            if($(".ev-passSave").is(".activation")){//如果按钮可以点击
                commLog.creatEvent({"id":153,"url":window.location.href,"keyword":"找回密码的提交"});
                var pass1 = $(".ev-newPas").val();//新密码
                var pass2 = $(".ev-conPas").val();//确认密码
                if(pass1!=pass2){
    ...
    quiLog:function(){
        ...
        $(".phoneLogin").off("click").on("click",function(){
            commLog.creatEvent({"id":3,"url":window.location.href,"keyword":"手机快捷登录"});
            $(".ev-login").css("display","none");
            $(".ev-quickLogin").css("display","block");
            t.quickCheck();//快速登录校验
            commLog.createBrowse(8, "登录注册-手机快捷登录页");
            $(".authorityBg").css("display","block");
            $(".registerAllinmd").unbind("click").bind("click",function(){
    ...
    quiCheckPhone:function(){
        ...
        if(isPhone==false){
            ...
            $(".registerAllinmd").addClass("none");
            //注册唯医通行证不显示时，即账号存在
            //$('.ev-quiCode').attr("disabled",false);
        }
    },
	/*
	 * 校验账户，用户未注册时为false
            ...
            t.ajaxFn({
                ...
                fn: function(data) {
                    ...
                    if(status==false){ //表示注册
                        ...
                        $('.ev-quiCode').attr("disabled",false).parent().removeClass("login-qui-disabled");
                    }else{
                        t.sendCode($(".ev-quickCode"),$(".ev-quiNum"),2);
                        $(".registerAllinmd").removeClass("none");
                        //注册唯医通行证显示时，即账号不存在
                        $('.ev-quiCode').attr("disabled",true).parent().addClass("login-qui-disabled");
                    }
                }
            });
        }
    ...
    loginBtn:function(){
        ...
        $(".ev-loginBtn").off("click").on("click",function(){
            commLog.creatEvent({"id":2,"url":window.location.href,"keyword":"登录提交"});
            if($(this).is(".activation")){
    ...
    weixin:function(){
        ...
        $(".weixin").off("click").on("click",function(){
            ...
            loginAbout.login.exit();
            comm.maskBackground.show("rgba(0,0,0,.6)");
            var iframHtml = "<section class='weixinCloseContent'><section class='weixinClose'></section></section><iframe src=\"//open.weixin.qq.com/connect/qrconnect?appid=wx3b347620d468cd89&redirect_uri=https%3a%2f%2fwww.yi-ding.net.cn%2fcall%2fyiding%2finteract%2fweixinLogin%2f?url="+t.isValid.weiUrl+"&response_type=code&scope=snsapi_login&state=START#wechat_redirect\" class=\"weixinLogin\" ></iframe>";
            $(".yd-maskBackground").html(iframHtml);
            $(".weixinClose").on("click",function(){
    ...
    bidWeiCheckPhone2:function(){
        var t = this;
        t.isValid.bidWeiPhone = true;
        var phone = $(".ev-bindWeixinNum");
        var phoneReg = /^(127|128|13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/;
        var emailReg = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
        var isPhone = (phoneReg.test(phone.val()))||(emailReg.test(phone.val()));
        if(isPhone==false){
            t.toast("请输入正确的手机号或邮箱",300,1500);
            t.isValid.bidWeiPhone = false;
        }else{
            t.ajaxFn({
            ...
            t.ajaxFn({
                ...
                fn: function(data) {
                    var status = data.responseObject.responseStatus;
                    if(status){ //表示没有注册
                        t.toast("账号不存在",300,1500);
                        t.isValid.bidWeiPhone = false;
                    }else{
                        //将用户的ID存在变量中
                        t.isValid.weixinCusId=data.responseObject.responseData.customerUnite.customerId;
                    }
                }
            });
        }
    ...
    bidWeiCheckCode:function(){
        ...
        var passWord = $(".ev-bindWeixinCode");
        var passReg = /^[0-9a-zA-Z]*$/g;
        var len = passWord.val().length;
        var isPass = passReg.test(passWord.val());
        if(isPass){
    ...
    bidWeiBtn:function(){
        ...
        $(".ev-bindWeixinLogin").off("click").on("click",function(){
            //commLog.createBrowse(11, "微信登录-账号绑定");
            if($(this).is(".activation")){
                commLog.creatEvent({"id":155,"url":window.location.href,"keyword":"微信授权登录，微信绑定医鼎账号的提交"});
                t.ajaxFn({
                    url: t.path.weixinBind,
                    type:"post",
                    async:false,
                            ...
                            t.ajaxFn({
                                ...
                                fn: function(data) {
                                    ...
                                    if(status){
                                        localStorage.setItem("userState","true");
                                        localStorage.setItem("userId",t.isValid.fidCustomerId);
                                        console.log(t.isValid.fidCustomerId);
                                        loginAbout.changeHead();
                                        location.reload();
                                        window.location.href=localStorage.getItem("weiUrl");
                                    }
                ...
                //if($(this).is(".activation")){
                //    var phone = $(".ev-bindWeixinNum").val();
                //    var pass = $(".ev-bindWeixinCode").val();
                //    $.ajax({
                //        type:"post",
                //        url:t.path.login,
                //        data: {
                ...
                //                if(status){
                //                    localStorage.setItem("userAuth",userAuth);
                //                    localStorage.setItem("userId",t.isValid.weixinCusId);//保存微信绑定的用户ID
                //                    loginAbout.login.exit();
                //                    loginAbout.changeHead();
                //                    localStorage.setItem("userState","true");
                //                    t.increment();
                    ...
                    fn: function(data) {
                        ...
                        if(codeNum||codeNum==0){
                            var timer = setInterval(function () {
                                ...
                                $(".ev-bindWeixinReturn").off("click").on("click",function(){
                                    clearTimeout(timer);
                                    $(".validate").text("点击获取验证码");
                                    $(".validate").css("color","#73859e");
                                    window.location.href=localStorage.getItem("weiUrl");
    ...
    allinBingLogin:function(){
        var t = this;
        $(".ev-allinCaos").on("click",function(){
            //commLog.creatEvent({"id":6,"url":window.location.href,"keyword":"CAOs登录"});
            //commLog.createBrowse(10, "登录注册-CAOS登录页");
        ...
        $('.ev-caosPhone').on('blur',function(){
            t.checkCAOSLogin2();
            t.activeCAOSLogin();
        })
        $('.ev-caosPhone').off('keyup').on('keyup',function(){
            t.checkCAOSLogin();
            t.activeCAOSLogin();
        })
        $('.ev-caosPass').on('blur',function(){
            t.checkCAOSPass2();
            t.activeCAOSLogin();
        })
        $('.ev-caosPass').off('keyup').on('keyup',function(){
            t.checkCAOSPass();
            t.activeCAOSLogin();
        })
        $('.ev-CAOSlOgin .returnText').on('click',function(){
            $('.ev-login').show();
            $('.ev-CAOSlOgin').hide();
        })
    },
    //检验CAOS账号
    checkCAOSLogin2:function(){
        var t = this;
        t.isValid.phone = true;
        var phone = $(".ev-caosPhone");
        var emailReg = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
        var phoneReg = /^(127|128|13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/;
        var isPhone = (phoneReg.test(phone.val()))||(emailReg.test(phone.val()));
        t.isValid.rightPhone = phoneReg.test(phone.val());
    ...
    //检验CAOS密码
    checkCAOSPass:function(){
        var t = this;
        t.isValid.pass = true;
        var passWord = $(".ev-caosPass");
        var passReg = /^[0-9a-zA-Z]*$/g;
        var len = passWord.val().length;
        var isPass = passReg.test(passWord.val());
        if(isPass){
    ...
    //激活CAOS登录按钮
    activeCAOSLogin:function(){
        var t = this;
        var phoneTxt = $(".ev-caosPhone").val();
        var passTxt = $(".ev-caosPass").val();
        if(phoneTxt&&passTxt){
            if(t.isValid.phone&&t.isValid.pass){
                $(".ev-CAOSloginBtn").addClass("activation");
                t.loginCAOSBtn();
            }else{
                $(".ev-CAOSloginBtn").removeClass("activation");
                $(".ev-CAOSloginBtn").unbind("click");
            }
        }else{
            $(".ev-CAOSloginBtn").removeClass("activation");
            $(".ev-CAOSloginBtn").unbind("click");
     ...
     * CAOS用户登录
     */
    loginCAOSBtn:function(){
        var t = this;
        $(".ev-CAOSloginBtn").off("click").on("click",function(){
            commLog.creatEvent({"id":2,"url":window.location.href,"keyword":"CAOS登录提交"});
            if($(this).is(".activation")){
                var phone = $(".ev-caosPhone").val();
                var pass = $(".ev-caosPass").val();
                $.ajax({
                    type:"post",
                    url:t.path.login,
                    data: {
